...
- For data that isn't an `InventoryPetSlot` (e.g. trade-menu pets, which carry only a serialized path + rarity — including the partner's snapshot), use `PetTooltipInfo.FromPrefab( petPrefabPath, rarity, fallbackName )`. It resolves `GameObject.GetPrefab(path)` → `PetComponent` and applies the identical rarity scaling as `FromSlot` (`FromSlot` now just delegates to it).
- The trade menu (`Code/UI/PetTradePanel.razor`) wires it via `SetHoveredTradePet( hoverKey, pet )` / `ClearHoveredTradePet`, and a `Tick()` override hides the tooltip if the panel closes mid-hover (so `onmouseout` not firing can't leave it stuck).
- The Pet Forge (`Code/UI/PetMergePanel.razor`) wires it via `ShowPetTooltip( slotIndex )` on the library cards and `ShowSlotTooltip( pet )` on the 3 merge slots, with `HidePetTooltip` on mouse-out. Its view-models carry the real inventory `SlotIndex`, so it reuses `FromSlot`. `ShowSlotTooltip` skips while `isMerging` (slots animate / result has no stable slot), and a `petTooltipActive` flag lets `Tick()`/`Close()` clear a stuck tooltip when the panel closes mid-hover.
...
- `Code/UI/PerfExplorerPanel.razor` — full **interactive menu** (child `Panel`, `pointer-events: all`) with a segmented **tab bar** (`PerfTab` enum, `selectedTab` is static so it persists): a **Performance** tab (FPS bar chart, collapsible detail sections, clickable bisect/culling toggles) and a **Project Explorer** tab (editor-style scene hierarchy: collapsible tree from `Scene.Children`, indent guides, name filter, Expand All / Collapse All, click name or ON/OFF pill to toggle `GameObject.Enabled`; caret toggles expand). Opening it puts the cursor up (it's a menu) — gated on `IsExplorerOpen`, which is in `PlayerHud.IsAnyMenuOpen` + BuildHash.
...
### Project Explorer tab notes
- Walks `GameObject.Children` (includes disabled objects), NOT `GetAllObjects` (enabled-only). Tree expand state is a static `HashSet<Guid>`; collapsed by default (only `Scene.Children` roots shown until expanded). Expand All / Collapse All populate/clear that set. The filter does a flat recursive name match (no tree while filtering). Rows capped at `MaxRows` (600) — use the filter or collapse branches to narrow. Clicking the name or the ON/OFF pill sets `GameObject.Enabled`; the caret toggles expand only. (`BrowserRow` is a `class` and `PerfTab` is an `enum` in `@code` — Razor codegen rejects a `record` there.)
